...
def index():
    ...
    if drn>0:
        ...
        if os.path.isdir(dr3):
            ...
            ff.close()
    ######
    #bcdata="Ramesh|Male}12-05-1999|56,tgb,5tff, Salem|624845|9638527415|ramesh2551cvd@gmail.com|10-98-BPPS,12-95-BPPS|UG-BCA-2013-88|PG-MCA-2015-85"
...
def reg1():
    ...
    if request.method=='POST':
        ...
        if myresult==0:
            ...
            sql = "INSERT INTO sw_user_register(id,name,mobile,email) VALUES (%s,%s,%s,%s)"
            val = (maxid,name,mobile,email)
...
def walletchain(uid,uname,bcdata):
    ############
...
def register():
    ...
    if request.method=='POST':
        ...
        if myresult==0:
            ...
            mycursor.execute("update sw_user_register set name=%s,middle_name=%s,last_name=%s,mobile=%s,email=%s,gender=%s,dob=%s,username=%s,password=%s,register_date=%s,upi_code=%s where id=%s",(name1,name21,name31,mobile1,email1,gender1,dob11,uname,pass11,rdate1,ucode1,uid))
            mydb.commit()
...
def upload():
    ...
    return render_template('upload.html',msg=msg,data=data)
...
#Lookup Substitution
def getdata(un,web_state,upi,qd):
    mess=""
    rdate=[]
    ky=un
    obj=AESCipher(ky)
    ...
    if web_state=="Trusted":
        ...
        up+="&pass="+pw
        s1="1"  
        page=web_url+"/store.php?"+up
        mess="Dear "+name+", You are registering "+web_url+", Username:"+us+", Password:"+pw
        bdata="Trusted, "+web_url+", Username:"+us+", Password:"+pw
        ...
        now = date.today() #datetime.datetime.now()
        rdate=now.strftime("%d-%m-%Y")
        rtime=rdate=now.strftime("%H:%M:%S")
        dtime=rdate+", "+rtime
        ...
        val = (maxid,un,web_url,web_state,us,pw,dtime)
        mycursor.execute(sql, val)
        mydb.commit()
        
        walletchain(uid,un,bdata)

        
    else:
        up="page=1"
        na1=['Raj','Gopi','Sheela','Nisha','Vijay','Kumar']
        xn1=randint(1,6)
        xn2=xn1-1
        name=na1[xn2]           
        ...
        bdata="Untrusted, "+web_url+", Username:"+us+", Password:"+pw
        walletchain(uid,un,bdata)
...
def verify():
    ...
    if act=="1":
        ...
        query=getdata(un,web_state,upi,qd)
        mess=query[0]
        email=query[1]
    ...
    return render_template('verify.html',msg=msg,act=act,upi=upi,qd=qd,wu=wu,mess=mess,email=email)
    

@app.route('/signup_verify', methods=['GET', 'POST'])
def signup_verify():
    msg=""
    act=request.args.get("act")
    ...
    web_state=""
    mycursor = mydb.cursor()
    mycursor.execute("SELECT count(*) FROM sw_webservice where service_code=%s",(secretcode,))
    cnt = mycursor.fetchone()[0]
    if cnt>0:
        mycursor.execute("SELECT * FROM sw_webservice where service_code=%s",(secretcode,))
        data = mycursor.fetchone()
    ...
    return render_template('signup_verify.html',msg=msg,act=act,un=un,s1=s1,page=page,secretcode=secretcode,mess=mess,email=email1)
